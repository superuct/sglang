...
- `weight_versions` holds one span per version change the request lived through, so the usual case is a single span. Ranges are half-open `[start, end)` over output-token indices, prompt excluded. The older `weight_version` field keeps its meaning by reporting the newest span.
